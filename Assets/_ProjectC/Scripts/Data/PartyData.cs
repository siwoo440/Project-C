...
public sealed class PartyData : ScriptableObject
{
    ...
    public IReadOnlyList<CharacterData> Members => members; // 파티 캐릭터 목록 조회
    public int MemberCount => members.Count; // 현재 파티 인원 조회

    public bool ContainsCharacter(CharacterData character)
    {
        return character != null && members.Contains(character); // 파티 포함 여부 반환
    }

    public bool IsValidParty()
    {
        if (members.Count < 1 || members.Count > MaxPartySize) // 파티 인원 범위 확인
        {
        ...
        foreach (CharacterData member in members) // 파티원 순회
        {
