...
public sealed class ExplorationPrototypeBootstrap : MonoBehaviour
{
    ...
    private void CreatePlayer(ExplorationSessionManager sessionManager)
    {
        ...
        playerObject.transform.position =
            sessionManager.GetPlayerSpawnPosition(new Vector3(-3.5f, -2.7f, 0f));

        SpriteRenderer spriteRenderer =
            playerObject.GetComponent<SpriteRenderer>();

        spriteRenderer.sprite = runtimeSquareSprite;
        spriteRenderer.color = new Color(0.2f, 0.7f, 1f, 1f);
    ...
    private void CreateEncounters(ExplorationSessionManager sessionManager)
    {
        ...
        Array.Sort(
            encounterData,
            (left, right) =>
                string.CompareOrdinal(left.EncounterId, right.EncounterId));

        for (int index = 0; index < encounterData.Length; index++)
        {
            EncounterData data = encounterData[index];

            if (data == null ||
                !data.IsValidData() ||
                sessionManager.IsEncounterCleared(data.EncounterId))
            {
                continue;
            }

            GameObject encounterObject = new GameObject(
    ...
    private void CreateHud(ExplorationSessionManager sessionManager)
    {
        ...
        instructionRect.sizeDelta = new Vector2(820f, 130f);
        instructionRect.anchoredPosition = new Vector2(24f, -24f);

        instructionText.text =
            "32일차 탐사 테스트\n" +
            "WASD / 방향키 이동 · 색상 사각형에 닿으면 전투\n" +
            "승리: 조우 제거 · 도주/패배: 조우 유지";
        ...
        countRect.sizeDelta = new Vector2(450f, 80f);
        countRect.anchoredPosition = new Vector2(-24f, -24f);

        int totalCount =
            Resources.LoadAll<EncounterData>("Encounters").Length;

        countText.text =
            $"클리어 {sessionManager.ClearedEncounterIds.Count} / {totalCount}";
    }
    ...
    private static Color GetEncounterColor(int index)
    {
        switch (index % 3)
        {
            case 0:
            ...
            default:
                return new Color(0.75f, 0.2f, 0.65f, 1f);
        }
    }
}
