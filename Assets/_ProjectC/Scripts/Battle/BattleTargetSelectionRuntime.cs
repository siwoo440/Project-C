...
public sealed class BattleTargetSelectionRuntime : MonoBehaviour // 카드 대상 선택 관리자
{
    ...
    [SerializeField] private BattleCardEffectResolver effectResolver; // 카드 효과 관리자
    ...
    public bool TrySelectTarget(BattleTargetView targetView) // 전투 대상 선택 시도
    {
        ...
        if (effectResolver == null) // 카드 효과 관리자 연결 확인
        {
            Debug.LogError("Battle card effect resolver is missing.", this); // 효과 관리자 누락 오류
            return false; // 대상 선택 실패 반환
        }

        if (effectResolver.CanResolve(cardToUse, selectedUnit) == false) // 효과 적용 가능 여부 확인
        {
            Debug.LogWarning($"Card effect cannot target this unit: {cardToUse.CardName}", this); // 효과 적용 불가 경고
            return false; // 대상 선택 실패 반환
        }
        ...
        bool resolved = effectResolver.TryResolve(cardToUse, selectedUnit); // 선택 대상에게 효과 적용

        if (resolved == false) // 효과 적용 결과 확인
        {
            Debug.LogError($"Card effect resolution failed: {cardToUse.CardName}", this); // 효과 적용 실패 오류
            return false; // 대상 선택 실패 반환
        }
